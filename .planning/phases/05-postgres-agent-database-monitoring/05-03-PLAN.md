...
must_haves:
  ...
  artifacts:
    - path: "postgres-agent/src/collectors/log_parser.py"
      ...
      exports: ["tail_postgres_log", "parse_log_line", "LogCollector"]
...
<task type="auto">
  <name>Task 4: Update collectors __init__.py to export log_parser</name>
  <files>
    postgres-agent/src/collectors/__init__.py
  </files>
  <action>
Update the collectors __init__.py to also export log_parser functions for consistency.

Update `postgres-agent/src/collectors/__init__.py` to include log_parser exports:
```python
...
Note: This updates the __init__.py that was created in Plan 05-02 to also export the log_parser module that this plan creates.
  </action>
  <verify>
    - collectors/__init__.py includes log_parser imports
    - __all__ includes tail_postgres_log, parse_log_line, LogCollector
  </verify>
  <done>collectors/__init__.py exports log_parser functions for package consistency</done>
</task>
...
7. collectors/__init__.py exports log_parser functions
</verification>
...
- log_parser exported from collectors package
</success_criteria>
